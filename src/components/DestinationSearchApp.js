...
const destinationsList = [
  ...
  // }
export default class DestinationSearchApp extends Component {
  constructor(props) {
    super(props)
  
    this.state = {
       search : "",
       researchDestinationList: destinationsList
    }
  ...
  onSearchHandler(event){
    this.setState({search: event.target.value},()=>{
      ...
      const filteredDestinationList= destinationsList.filter(eachItem => eachItem.name.includes(search))
      this.setState({researchDestinationList : filteredDestinationList})
      console.log(destinationsList)
    })
    
  }
  render() {
    const {researchDestinationList} = this.state
    return (
      <div className="main-container">
        {/* <div>{this.state.search}</div> */}
        <h1>Destination Search</h1>
        <input type="text" onChange={this.onSearchHandler} value={this.state.search}/>
        <div className="destination-container">
          {researchDestinationList.map(eachItem=>(
            <LocationComponent item={eachItem} key={eachItem.id}/>
          ))}
        </div>
